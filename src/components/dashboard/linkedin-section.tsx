...
import { useLinkedInPosts, useLinkedInRanking } from "@/hooks/use-linkedin";
...
export function LinkedInSection({ brand, days }: { brand: string; days: number }) {
  ...
  const { data, isLoading } = useLinkedInPosts({ brand, sort });
  const { data: ranking } = useLinkedInRanking(now.getMonth() + 1, now.getFullYear());
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-base font-semibold tracking-tight">LinkedIn</h2>
        <p className="text-sm text-muted">Contenido orgánico, seguidores y rendimiento de la Página de empresa</p>
      </div>

      <FollowersSection brand={brand} network="LINKEDIN" days={days} accentColor="var(--accent)" />
      ...
      <Panel title="Publicaciones" description={`${data?.posts.length ?? 0} publicaciones encontradas`}>
        {isLoading ? (
          ...
          <p className="text-sm text-muted py-6 text-center">
            Todavía no hay publicaciones sincronizadas de LinkedIn para esta marca. Conecta las credenciales en Configuración y sincroniza con &ldquo;Actualizar ahora&rdquo;.
          </p>
        )}
      </Panel>
