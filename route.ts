...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Red de seguridad: si algo revienta a mitad de camino (timeout de una
    // API externa, error inesperado, etc.), igual devolvemos JSON válido en
    // vez de dejar la respuesta a medias — eso es lo que causaba el error
    // "Unexpected end of JSON input" en el botón "Actualizar ahora".
    console.error("Error en /api/sync:", err);
    return NextResponse.json(
      { synced: false, reason: err instanceof Error ? `Error interno: ${err.message}` : "Error interno desconocido durante la sincronización." },
      { status: 500 }
    );
  }
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("Error en /api/sync (cron):", err);
    return NextResponse.json(
      { synced: false, reason: err instanceof Error ? `Error interno: ${err.message}` : "Error interno desconocido durante la sincronización." },
      { status: 500 }
    );
